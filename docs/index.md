...
Action Gateway 把 MySQL、Redis、Kubernetes、应用日志和审计查询能力暴露为 MCP tools，并用 API Key、policy、source、allowlist 和审计记录控制每一次调用。
...
- 让 Agent 查询应用日志索引和 Gateway 审计摘要。
...
2. [配置 Source 和 Allowlist](/guide/configure-sources)：把 MySQL、Redis、Kubernetes 和日志 Redis 接入 Gateway。
