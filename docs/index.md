...
2. [核心概念](/guide/concepts/)：理解 Principal、API Key、source、allowlist、access policy 和 audit event 的关系。
3. [Source 和 Allowlist](/configuration/sources-and-allowlists/)：把 MySQL、Redis、SLS 和 Kubernetes 接入 Gateway。
4. [权限和 agctl](/configuration/agctl/)：给调用方创建 principal、role binding 和 API Key。
5. [部署](/operations/deployment/)：在开发、测试或生产环境部署 Gateway。
6. [Codex / MCP Client](/clients/codex/)：把 Gateway 配置到 Codex 或其他 MCP Client。
