...
- **Allowlist**：这个工具是否被允许触达具体表、key 或 namespace/resource；SLS 日志查询由 access policy 约束到具体 `<project>/<logstore>`。
...
| `logs.query_sls_logs` | `query` | `sls_logstore` |
